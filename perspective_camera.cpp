...
PerspectiveCamera::PerspectiveCamera( const float min_x,
        ...
        min_y_{ min_y },
        max_y_{ max_y },
	s_{ s } ///distancia da posição da câmera ao plano 
{}
...
Ray PerspectiveCamera::getWorldSpaceRay( const glm::vec2 &pixel_coord ) const
{
    ...
    glm::vec3 C_{ (pixel_coord[0] + 0.5f) / static_cast< float >( resolution_[0] ) * width + min_x_,
                      (pixel_coord[1] + 0.5f) / static_cast< float >( resolution_[1] ) * height + min_y_,
                      -s_ };

    return Ray{ position_, 
                glm::normalize( onb_.getBasisMatrix() * (C_ - position_) ) };
}
